...
class TestWikiPage:
    ...
    def test_update_not_allowed(self, user, other):
        ...
        page.update(
            title="Updated", body="Updated", editor=other, write=PermissionLevel.PUBLIC, read=PermissionLevel.PUBLIC
        )
    ...
    def test_update_no_lockout(self, user):
        ...
        page.update(
            title="Updated", body="Updated", editor=user, write=PermissionLevel.OWNER_ONLY, read=PermissionLevel.PUBLIC
        )
        assert page.title == "Test"
    ...
    def test_can_write(self, request, before, after, reader_fixture, expected, user, owner):
        page = WikiPage.create(title="Test", body="Test", editor=user, owner=owner, write=before)
        reader = None if reader_fixture is None else request.getfixturevalue(reader_fixture)
